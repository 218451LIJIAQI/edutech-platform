...
import { BookOpen, Clock, Calendar, CheckCircle, Star } from 'lucide-react';
...
import { getPreferredCourseEnrollments } from '@/utils/enrollment';
...
const MyCoursesPage = () => {
  ...
  useEffect(() => {
    ...
    const loadEnrollments = async () => {
      setIsLoading(true);
      try {
        const [enrollmentItems, reviews] = await Promise.all([
        ...
        if (isActive) {
          ...
          setReviewedCourseIds(
            new Set(
              reviews
                .map((review) => review.enrollment?.package?.course?.id)
        ...
        if (isActive) {
          clientLogger.error('Failed to fetch enrollments:', error);
  ...
  const courseEnrollments = getPreferredCourseEnrollments(enrollments);
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-primary-50/10 to-indigo-50/20 relative">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 relative">
        ...
        {/* Filter Tabs */}
          ...
          >
            All Courses ({courseEnrollments.length})
          </button>
          <button
            type="button"
          ...
          >
            In Progress ({courseEnrollments.filter((e) => e.progress < 100).length})
          </button>
          <button
            type="button"
      ...
      {filteredEnrollments.length > 0 ? (
        <div className="space-y-6">
          {filteredEnrollments.map((enrollment) => {
            ...
            const hasReviewedCourse = reviewedCourseIds.has(course.id);
            ...
            const primaryActionLabel = isCompleted
              ? hasReviewedCourse
                ...
                : 'Leave Review'
              : 'Continue Learning';
            const primaryActionHref = isCompleted && !hasReviewedCourse
                ...
                <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
                  ...
                  <div className="lg:col-span-1 flex flex-col justify-center gap-3">
                    <Link
                      ...
                      aria-label={`${primaryActionLabel}: ${course.title}`}
                      className="btn-primary text-center font-bold py-3 rounded-xl shadow-lg hover:shadow-xl transition-all transform hover:scale-105 inline-flex items-center justify-center gap-2"
                    >
                      {isCompleted && !hasReviewedCourse ? (
                    ...
                    {isCompleted && hasReviewedCourse && (
                      <div className="flex items-center justify-center gap-2 text-amber-700 text-sm font-bold bg-amber-50 px-4 py-3 rounded-xl border-2 border-amber-200 shadow-md">
