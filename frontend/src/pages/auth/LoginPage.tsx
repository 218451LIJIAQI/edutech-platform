...
const LoginPage = () => {
  ...
  useEffect(() => {
    if (error) {
      const timer = setManagedTimeout(() => setError(''), 5000);
      return () => clearManagedTimeout(timer);
    }
  }, [clearManagedTimeout, error, setManagedTimeout]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const previousPath =
        previousLocation?.pathname && previousLocation.pathname !== '/login'
      ...
      setError(extractErrorMessage(error, 'Login failed, please try again'));
    } finally {
      setIsLoading(false);
    }
  };
